...
def decompose(tree, single_tree=False):
    ...
    for node in tree.traverse_postorder(leaves=False):
        if node.tag == 'D':
            ...
            not_delete = right if len(left.s) < len(right.s) else left # the other one 
            not_delete_labels = set(tree.extract_subtree(not_delete).labels(leaves=True, internal=False))
            # print("\nNot_delete labels:", not_delete_labels) 
            # print("Delete labels", set(tree.extract_subtree(delete).labels(leaves=True, internal=False)))
            # print("Delete tree", tree.extract_subtree(delete).newick())
            # # sd :: subtree_of_delete_with_leaves_not_in_not_delete
            sd = tree.extract_subtree(delete).extract_tree_without(not_delete_labels, suppress_unifurcations=True)
            # print("Appending labels:", set(sd.labels(internal=False)))
            # print(sd.root.label in not_delete_labels)
            if not single_tree:
                out.append(tree.extract_subtree(delete).extract_tree_without({''}, suppress_unifurcations=True))
                # out[-1].suppress_unifurcations()
            node.remove_child(delete)
            if not sd.root.label in not_delete_labels:
                node.add_child(sd.root)
            # print("Current Tree:", tree.newick()) 
    # print("Is empty there?", '' in tree.labels(internal=False))
    tree = tree.extract_tree_without({''}, suppress_unifurcations=True)
    tree.suppress_unifurcations() # all the duplication nodes will be unifurcations
    # print("Is empty still there?", '' in tree.labels(internal=False))
    out.append(tree)
    # print("Final tree:", tree.newick()) 
    return out
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='====================== DISCO v1.3.1 ======================\nReadds a subtree of the pruned tree to avoid duplications. Dup and loss weights are random.')
